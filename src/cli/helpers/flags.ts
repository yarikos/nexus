...
export type Input<T extends Parser.flags.Output> = {
...
export type Definition<T> = {
  ...
  (
    options: ({ required: true } | { default: Parser.flags.Default<T> }) &
...
const _enum = <T = string>(
  opts: Parser.flags.EnumFlagOptions<T>
): IOptionFlag<T> => {
  return build<T>({
    parse(input) {
      if (!opts.options.includes(input))
        throw new Error(
          `Expected --${this.name}=${input} to be one of: ${opts.options.join(
            ', '
          )}`
        )
      return input
...
export const version = (
  opts: Partial<Parser.flags.IBooleanFlag<boolean>> = {}
) => {
